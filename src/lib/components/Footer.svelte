<footer>
	<p class="social-links">
		<a href="https://github.com/dmdez" aria-label="Github">
			<svg width="24" height="24" viewBox="0 0 100 100"
				><path
					fill-rule="evenodd"
					clip-rule="evenodd"
					d="M48.854 0C21.839 0 0 22 0 49.217c0 21.756 13.993 40.172 33.405 46.69 2.427.49 3.316-1.059 3.316-2.362 0-1.141-.08-5.052-.08-9.127-13.59 2.934-16.42-5.867-16.42-5.867-2.184-5.704-5.42-7.17-5.42-7.17-4.448-3.015.324-3.015.324-3.015 4.934.326 7.523 5.052 7.523 5.052 4.367 7.496 11.404 5.378 14.235 4.074.404-3.178 1.699-5.378 3.074-6.6-10.839-1.141-22.243-5.378-22.243-24.283 0-5.378 1.94-9.778 5.014-13.2-.485-1.222-2.184-6.275.486-13.038 0 0 4.125-1.304 13.426 5.052a46.97 46.97 0 0 1 12.214-1.63c4.125 0 8.33.571 12.213 1.63 9.302-6.356 13.427-5.052 13.427-5.052 2.67 6.763.97 11.816.485 13.038 3.155 3.422 5.015 7.822 5.015 13.2 0 18.905-11.404 23.06-22.324 24.283 1.78 1.548 3.316 4.481 3.316 9.126 0 6.6-.08 11.897-.08 13.526 0 1.304.89 2.853 3.316 2.364 19.412-6.52 33.405-24.935 33.405-46.691C97.707 22 75.788 0 48.854 0z"
					fill="currentColor"
				/></svg
			>
		</a>
		<a href="mailto:dmmendez@gmail.com" title="Email">
			<svg width="24" height="24" viewBox="0 0 122.88 88.86"
				><title>email</title><path
					d="M7.05,0H115.83a7.07,7.07,0,0,1,7,7.05V81.81a7,7,0,0,1-1.22,4,2.78,2.78,0,0,1-.66,1,2.62,2.62,0,0,1-.66.46,7,7,0,0,1-4.51,1.65H7.05a7.07,7.07,0,0,1-7-7V7.05A7.07,7.07,0,0,1,7.05,0Zm-.3,78.84L43.53,40.62,6.75,9.54v69.3ZM49.07,45.39,9.77,83.45h103L75.22,45.39l-11,9.21h0a2.7,2.7,0,0,1-3.45,0L49.07,45.39Zm31.6-4.84,35.46,38.6V9.2L80.67,40.55ZM10.21,5.41,62.39,47.7,112.27,5.41Z"
					fill="currentColor"
				/></svg
			>
		</a>
	</p>
	<p>
		Built with 🔨
		<br />
		<a href="https://github.com/dmdez/dmdez.github.io" target="_blank">
			SvelteKit, CSS3 and D3 (timeline) ⤴
			<!-- <ExternalLinkIcon mx="6px" mb="3px" /> -->
		</a>
	</p>
</footer>

<style lang="postcss">
	.social-links {
		display: inline-flex;
		gap: 1rem;
	}

	.social-links a {
		color: var(--color-primary);
		border: 1px solid currentColor;
		display: block;
		border-radius: 100%;
		text-align: center;
		padding: 0.3rem;
	}

	.social-links a svg {
		display: block;
	}
	footer {
		padding: 3rem 0 5rem;
		text-align: center;
	}
</style>
